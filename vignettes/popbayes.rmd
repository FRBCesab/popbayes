...
modeled is the rate of increase between successive years (or any other time unit 
...
In addition to the fields `location`, `species`, `year`, and `counts`, a fourth 
...
- `T` or `G`, a confidence interval will be computed automatically by the 
...
- `conversion_fact`: the multiplicative factor used to convert an aerial count 
into an equivalent ground count or vice versa.
...
knitr::kable(conversion_data[ , c(3, 6, 7, 8)], 
...
automatically retrieve the values of `pref_field_method` and `conversion_fact` 
from the `conversion_data` data set. But for other species, users 
**need to provide** the information themselves when running `format_data()`. 
These values can be provided as additional columns of the count data set. Care 
...
to create an independent additional table similar to `conversion_data` and to 
pass it to the function `format_data()` as the argument `info`. 
...
columns in data (if `info` is not provided), and eventually to the 
`conversion_data` table of the package (when the other two sources are missing). 
...
`conversion_data` table of the package. However, you can easily construct `info` 
from a copy of `conversion_data`, which additionally provides a ready template. 
It suffices to add any species not already in `conversion_data` as shown below.

Let's assume that we have counts of _Taurotragus oryx_ and _Taurotragus derbianus_ 
in addition to other species present in the package `conversion_data` table. 
We can construct `info` as follows. 
...
The field `category` of the `conversion_data` table indicates which species 
belong to each.
...
The first thing that the function `format data()` does is to check the validity 
of the content of the different fields of the count data set. Here we will 
explore our data to avoid errors while using the function `format data()`.

In particular, we need to check `location` and `species` spelling, `year` and 
`counts` field format, and the `stat_method` and `field_method` categories.
